...
const HomePage: React.FC<Props> = ({ lang, setView }) => {
  ...
  return (
    <div className="animate-fade-in space-y-8 md:space-y-12 pb-12">
      ...
      <section className="relative h-[250px] md:h-[400px] rounded-[1.25rem] md:rounded-[2.5rem] overflow-hidden shadow-2xl group">
        <img 
        ...
        <div className="absolute inset-0 bg-gradient-to-t md:bg-gradient-to-r from-[#1a2b4c] via-[#1a2b4c]/90 md:via-[#1a2b4c]/60 to-transparent flex items-center justify-center md:justify-start px-6 md:px-12">
          <div className="max-w-lg space-y-2.5 md:space-y-5 text-center md:text-start">
            ...
            {/* Stats Overlay */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-x-3 gap-y-1 md:gap-8 pt-3 md:pt-10 border-t border-white/10">
              {[
                ...
                <div key={i} className="space-y-0">
                  <p className="text-base md:text-2xl font-black text-white">{stat.value}</p>
                  <p className="text-[6px] md:text-[8px] text-white/40 font-black uppercase tracking-widest">{stat.label}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Flash Sale Section */}
      <section className="space-y-10 md:space-y-16 bg-[#c4a76d]/5 dark:bg-[#c4a76d]/5 p-10 md:p-24 rounded-[2rem] md:rounded-[4rem] border border-[#c4a76d]/10 relative overflow-hidden">
            ...
            <div className="flex overflow-x-auto gap-4 md:gap-8 no-scrollbar pb-6 scroll-smooth" id="offers-scroll">
              {offerProducts.map((p) => (
                ...
                >
                  ...
                  <div className="p-4 md:p-6 space-y-2 flex-1 flex flex-col justify-between">
      ...
      <section className="relative overflow-hidden bg-[#1a2b4c] text-white p-8 md:p-16 lg:p-20 rounded-2xl md:rounded-[3rem] shadow-2xl border-b-4 md:border-b-[6px] border-[#c4a76d]">
        ...
        <div className="relative z-10 max-w-xl mx-auto text-center space-y-3 md:space-y-5">
          ...
          <div className="space-y-3">
            <button 
              onClick={() => setView('offers')}
              className="w-fit mx-auto bg-white text-[#1a2b4c] px-6 md:px-12 py-2.5 md:py-4 rounded-lg md:rounded-2xl font-black text-xs md:text-lg shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-2 md:gap-4"
            >
